...
```{r}
set.seed(993)
RL_splits <- initial_split(RL_Lasso_df, prop = 0.7)
...
```{r}
set.seed(5)
RL_resamples <- vfold_cv(RL_Lasso_df, v = 10)
...
results <- fit(RL_workflow_final, data = RL_Lasso_df) %>%
  tidy() %>%
  filter(estimate != 0)
print(results, n = 42)
```
...
**The Lasso model classified game outcome with 70.797% accuracy.**
