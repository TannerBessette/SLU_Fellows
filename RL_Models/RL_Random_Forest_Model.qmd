...
Select predictor variables that are numeric and not obviously correlated:
```{r}
team_diff1 <- team_diff %>% select(where(is.numeric), winner)
team_diff1 <- team_diff1 %>% select(-c("core_goals_diff", "winner_numeric",
                                       "winner_factor",
                                       "core_assists_diff",
                                       "core_score_diff"))
```
...
The Random Forest model predicted game outcome with 94.627% accuracy.
