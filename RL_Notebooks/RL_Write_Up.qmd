...
RL_numeric <- read.csv(here("RL_Datasets/RL_numeric.csv"))
```
...
A random forest model uses a subset of predictor variables from the dataset and creates a collection of "trees" to make its prediction. In this model, the predictor variables the model utilizes are core_shooting_percentage_diff, core_shots_diff, and positioning_time_behind_ball_diff. Each branch gives a variable and a number. When making predictions, we start at the top of the tree and move our way down. For each individual game the model predicts for, if each variable is greater than the given number, it continues down the tree to the left, and if it is less, it continues down the tree to the right.

Let's follow this process along using an example from my model. Taking one random game from the dataset, the random forest model will analyze if the shooting percentage difference is greater than/equal to -1.3, or if it is less than -1.3. If it is greater than -1.3, we follow the model down to the left and next examine if the core shot difference is greater than/equal to or less than -3. Had the shooting percentage difference in the game had been less than -1.3, we would have followed the tree down to the right side and examined how the core shot difference compared to 5. The model continues this process all the way down to the last leg where we classify the game as a win or a loss. In this circumstance, a 1 at the top of the bottom bubble represents a predicted win, and a 0 represents a predicted loss. 
...
An important thing to understand in the context of random forest (and statistical modeling in general) is the concept of bootstrapping. Bootstrapping is essentially sampling the set-aside training data with replacement. This process is useful for understanding bias and variance within the dataset. (Kaggle source)

The random forest approach makes use of bagging when creating its model. Bagging starts with creating different training subsets with replacement randomly (bootstrapping).(simplilearn source) Then, a base model is created and fit to each of these subsets of the training data. These models run independently of each other, and final predictions are determined by combining the predictions from all of the models. (Kaggle source) Bagging is one of the advantages of random forest models, because it reduces variations in predictions in the process of combining the results of multiple decision trees on different samples of the training data (edureka source)
...
**_Important side note:_** Bagging is relatively similar to boosting, but they do distinctly differ. The main difference between bagging and boosting is that bagging is performed with all sub-models being completely independent of each other during the process, while boosting is sequential. The goal of boosting is to combine a set of weak predictors into a strong predictor to minimize the model's training error. This is done by having each of the samples of training data created used to fit a model, and then the process continues sequentially. (IBM source) By training sequentially, the model gets stronger and stronger because it attempts to make up for the weaknesses of the model before it. The important thing to note in boosting is that each succeeding model is dependent on the previous model. (Kaggle source) The main difference between gradient boosting modeling and random forest modeling is that the gradient boosting models follow this process of being built additively one after another. (leonlok.uk source) 

Using the dataset with all of the net variables as well as all of the standard deviation variables, I created a random forest model to predict the winner of each game:
...
The test dataset is the rest of the data that was set aside separate from the data used to train the model. After we have built the model on the training data, it can be applied to the test data to get a more precise idea of how strong our model is. For this project, the random forest appraoch identified the best possible model to include a three predictor subset of the dataset. The three variables selected were all net variables: core_shooting_percentage_diff, core_shots_diff, and positioning_time_behind_ball_diff, and none of the standard deviation variables were selected.

The final step in this process was to get predictions on the game outcome of all the games in the RL_test dataset, and see how accurately the model performed:
